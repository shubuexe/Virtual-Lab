...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // NEW: Add this line to initialize the splash screen
  setupSplashScreen();
  ...
  // Initialize professional intro animation
  setupIntroAnimation();
});
...
function initCircuitLines() {
  ...
  for (let i = 0; i < numLines; i++) {
    ...
    if (isHorizontal) {
      ...
      const left = Math.random() * (maxWidth - width);
      ...
      const top = Math.random() * (maxHeight - height);
    ...
    line.style.animation = `fade-in-up 0.5s ease forwards ${0.1 + i * 0.1}s`;
    
    circuitLines.appendChild(line);
  }
}

// Function to handle splash screen animation
function setupSplashScreen() {
  // Check if we're coming from a simulator page
  if (document.referrer.includes('simulator.html') || 
      document.referrer.includes('amplifier.html') ||
      sessionStorage.getItem('hasSeenIntro')) {
    // Skip animation if coming from a simulator or if already seen
    const splashScreen = document.querySelector('.intro-animation');
    if (splashScreen) {
  ...
  // Set flag that user has seen the intro
...
function setupIntroAnimation() {
  ...
  // Proceed with intro animation only for fresh visits
  const experimentCards = document.querySelectorAll('.experiment-card');
...
// Function to get appropriate time-based greeting
...
// Function to initialize personalized welcome message
function setupWelcomeBanner() {
  // Get user's name from profile (or use default)
  const userName = document.getElementById('userName');
  const timeGreeting = document.getElementById('timeGreeting');
  
  // Display appropriate time-based greeting
  if (timeGreeting) {
